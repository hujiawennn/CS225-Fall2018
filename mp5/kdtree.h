...
class KDTree
{
  ...
  public:
    ...
    /*
    Helper function for calculating the distance between the two points.
    * @param m One point of two
    * @param n The other point of two
    * @return A int value indicating the distance between the two points.
    */
    double calDistance(const Point<Dim> & m,
    ...
    *
    * @param index inputs for a group of points and current dimension.
    * @return a pointer to the KDTreeNode.
    */
    KDTreeNode * KDTreeHelper(int left, int right, int dim);

    /**
    * destructor helper functions
    *
    * @param KDTreeNode to delete;
    */
    void clear(KDTree<Dim>::KDTreeNode * root);

    /**
    * quickSelect function
    *
    * @param index inputs for a group of points, k-th smallest element
    ...
    */
    ...
    /**
    * grop a list into two parts: those less than a certain element,
    * and those greater than or equal to the element.
    *
    * @param index inputs for a group of points, k-th smallest element
    * and current dimension.
    *
    * @return current position of the picked element.
    */

    int partition(int left, int right, int k, int dim);
    ...
    /**
    * helper function for find the nearest Neighbor
    * @param query The point we wish to find the cloe  neighbor to in the tree.
    * @param subroot The KDTreeNode we are exploring.
    * @param nearestNeighbor The current nearestNeighbor we have.
    * @param dim The current dimension we are using.
    * @return The closest piont to query under the node subroot.
    */
    Point<Dim> findNNHelper(const Point<Dim>& query, KDTreeNode* subroot,
                            Point<Dim>& nearestNeighbor, int dim) const;
  ...
  private:
    ...
    /** Internal representation, root, size and points in teh tree**/
    KDTreeNode *root;
    size_t size;
